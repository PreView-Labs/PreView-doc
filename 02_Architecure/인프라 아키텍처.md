...
사용자의 요청은 \*\*CloudFront(CDN)\*\*를 단일 진입점으로 하여 \*\*프론트엔드(S3)\*\*와 \*\*백엔드(EC2)\*\*로 라우팅됩니다. 백엔드 서버는 \*\*외부 LLM 서비스(OpenAI API 등)\*\*와 통신하여 AI 면접 기능을 수행합니다.
...
graph TB
    ...
    subgraph "CI/CD 파이프라인"
        ...
        JENKINS -->|FE 빌드 & 업로드| S3_BUCKET
        JENKINS -->|캐시 무효화| CLOUDFRONT
        JENKINS -->|BE 빌드 & 재기동| SPRING_CONT
    end

    subgraph "AWS 클라우드 (ap-northeast-2)"
        subgraph "네트워크/보안"
            ROUTE53[Route 53<br/>DNS: previewai.store]
            ACM[ACM<br/>SSL/TLS 인증서]
        end

        subgraph "CDN 계층"
            CLOUDFRONT[CloudFront<br/>Global Edge Network]
            
            ROUTE53 -->|Alias| CLOUDFRONT
            ACM -.->|HTTPS 적용| CLOUDFRONT
        end

        subgraph "프론트엔드 (Serverless)"
            S3_BUCKET[S3 Bucket<br/>정적 웹 호스팅]
    ...
    style LLM fill:#00A67E,color:#fff
```
...
자체 모델 호스팅 대신 외부 고성능 LLM API를 활용하여 서버 리소스 부하를 최소화했습니다.

| 구성 요소 | 기술 스택 | 역할 |
|:---:|:---:|:---|
| **Model** | **GPT-4o (OpenAI)** | 이력서 분석, 면접 질문 생성, 꼬리 질문 생성, 답변 피드백 |
...
학생/취준생 프로젝트 예산에 맞춰 \*\*프리티어(Free Tier)\*\*를 최대한 활용하고, 꼭 필요한 부분만 최소 비용으로 구성했습니다.
